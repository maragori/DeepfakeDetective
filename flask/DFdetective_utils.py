...
def cluster_faces(faces, resnet, verbose=True, plot_clusters=False):
    ...
    clt = DBSCAN(metric="euclidean", eps=0.65, min_samples=3, n_jobs=-1, leaf_size=10)
...
def init_detective_model(path, device):
    ...
    model.load_state_dict(torch.load(path + f'checkpoint-{14}.pth.tar',  map_location=torch.device('cpu'))['model'])
...
def generate_html(results_per_cluster, path, process_id):
    ...
    prediction_name = path.split('/')[-1]
    print(prediction_name)
    with open('templates/' + prediction_name + '_prediction.html', 'w') as f:
        f.write(file_content)


def cleanup(file_age, folder=None, part_name=None):
    now = time.time()

    old = glob.glob(os.path.join(folder, part_name))
    for old_thing in old:
        if os.stat(old_thing).st_mtime < now - (file_age*60): # all files older than X minutes
            if os.path.isdir(old_thing):
                shutil.rmtree(old_thing)
            elif os.path.isfile(old_thing):
                os.remove(old_thing)
            else:
                pass
...
def get_clusters(url, device, process_id):
    ...
    # clean up all files older than 15 min
    cleanup(file_age=15, folder='static', part_name="https*")
    cleanup(file_age=15, folder='templates', part_name='*prediction.html')
